...
namespace FAS.Material {
    public partial class ImptView : ImportDataView {
        ...
        private void fileBrowser1_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e) {
            ...
            try {
                var ipitm = XlsCommon.Read<UltraDbEntity.T_ERP_Material_Impt>(fileBrowser1.Text, DicKF);
        ...
        private bool ChkData(List<UltraDbEntity.T_ERP_Material_Impt> ds) {
            var bok = true;
            //检测是否存在同名供应商
            ds.ForEach(j => {
                j.Updator = j.Creator = this.CurUser;
                j.Reserved2 = j.Remark = string.Empty;
                j.CostPrice = j.CostPrice ?? 0;
            });
            ds.Where(j => string.IsNullOrEmpty(j.MaterialNo)).ToList().ForEach(j => {
                j.Remark = "物料编码不能为空";
                bok = false;
            });
            ds.Where(j => string.IsNullOrEmpty(j.MaterialName)).ToList().ForEach(j => {
                j.Remark += " 物料名称不能为空";
                bok = false;
            });
            ds.Where(j => j.SafeQty < 0).ToList().ForEach(j => {
                j.Remark += " 安全库存不能小于0";
                bok = false;
            });
        ...
        private void btnChk_Click(object sender, EventArgs e) {
            var ipitm = GridControl.GetDataSource<T_ERP_Material_Impt>();
        ...
        private void btnImp_Click(object sender, EventArgs e) {
            ...
            try {
                var ds = GridControl.GetDataSource<UltraDbEntity.T_ERP_Material_Impt>();
                ...
                var rd = SerNoCaller.Calr_Material_Impt.Add(ds);
                if (!rd.IsOK) {
                    MsgBox.ShowErrMsg(rd.ErrMsg); return;
                }

                //执行同步
                SerNoCaller.Calr_Material_Impt.ExecSql("exec P_FAS_SyncMaterialImpt");
