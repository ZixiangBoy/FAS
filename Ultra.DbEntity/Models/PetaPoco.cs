...
namespace PetaPoco
{
    ...
    public partial class Database : IDisposable
    {
        ...
        public Database(string connectionStringName)
        {
            ...
            _connectionString = connectionStringName;// ConfigurationManager.ConnectionStrings[connectionStringName].ConnectionString;
            _providerName = providerName;
            CommonConstruct();
        }
                    ...
                    using (var cmd = CreateCommand(_sharedConnection, ""))
                    {
                        ...
                        foreach (var i in pd.Columns)
                        {
                            ...
                            if (i.Value.ResultColumn)
                                continue;
                            //Don't insert TimeStamp columns
                            if (0 == string.Compare(i.Value.ColumnName, "TimeStamp", true) &&
                                i.Value.PropertyInfo.PropertyType == typeof(byte[]))
                            {
                                continue;
                            }
                            //Don't insert CreateDate/Update columns
                            if (0 == string.Compare(i.Value.ColumnName, "CreateDate") ||
                                0 == string.Compare(i.Value.ColumnName, "UpdateDate"))
                                continue;
        ...
        //                        if (index > 0)
        //                            sb.Append(", ");
        //                        if (0 == string.Compare(i.Value.ColumnName, "UpdateDate"))
        //                        { sb.AppendFormat("UpdateDate = GetDate()"); }
        //                        else
        //                        {
        ...
        //                        //Don't insert TimeStamp columns
        //                        if (0 == string.Compare(colname, "TimeStamp", true) )
        //                        {
        //                            continue;
        //                        }
        //                        //Don't insert CreateDate/Update columns
        //                        if (0 == string.Compare(colname, "CreateDate")
        //                            //||0 == string.Compare(i.Value.ColumnName, "UpdateDate")
        //                            )
        //                            continue;
        //                        // Build the sql
        //                        if (index > 0)
        //                            sb.Append(", ");
        //                        if (0 == string.Compare(colname, "UpdateDate"))
        //                        { sb.AppendFormat("UpdateDate = GetDate()"); }
        //                        else
        //                        {
        ...
        /// <param name="columns">The column names of the columns to be updated, or null for all</param>
        /// <returns>The number of affected rows</returns>
        public int Update(string tableName, string primaryKeyName, object poco, object primaryKeyValue, IEnumerable<string> columns, string extrawhr = "")
        {
            try
            {
                OpenSharedConnection();
                try
                {
                    using (var cmd = CreateCommand(_sharedConnection, ""))
                    {
                        ...
                        AddParam(cmd, primaryKeyValue, pkpi);
                        //Szy Add Here
                        if (!string.IsNullOrEmpty(extrawhr))
                        {
                            cmd.CommandText = cmd.CommandText + " and " + extrawhr;
                        }
                        DoPreExecute(cmd);
        ...
        //Szy Add Here
        public int UpdateExWhr(object poco, string exwhr)
        {
            var pd = PocoData.ForType(poco.GetType());
            return Update(pd.TableInfo.TableName, pd.TableInfo.PrimaryKey, poco, null, null, exwhr);
        }

        public int UpdatePartExWhr(object poco, IEnumerable<string> columns, string exwhr)
        {
            var pd = PocoData.ForType(poco.GetType());
            return Update(pd.TableInfo.TableName, pd.TableInfo.PrimaryKey, poco, null, columns, exwhr);
        }
                ...
                else if (!dstType.IsAssignableFrom(srcType))
                {
                    ...
                    else
                    {
                        return delegate(object src) {
                            ...
                            //}
                            //catch(Exception) {
                            //    bool dstnull = dstType.IsGenericType && dstType.GetGenericTypeDefinition() == typeof(Nullable<>);
                            //    bool srcnull = src.GetType().IsGenericType
                            //        && src.GetType().GetGenericTypeDefinition() == typeof(Nullable<>);
                            //    //src非可空
                            //    if (!srcnull)
                            //    {
                            //        return Convert.ChangeType(src, dstType.UnderlyingSystemType, null);
                            //    }
                            //    else
                            //        return Convert.ChangeType(src, dstType, null);
                            //}
