...
namespace FAS.Trade {
    public partial class SCTrdEdtView : DialogViewEx {
        ...
        private void SCTrdEdtView_Load(object sender, EventArgs e) {
            InitEvent();
            LoadData();
        }
        ...
        private void btnSplit_Click(object sender, EventArgs e) {
            var vw = new OdrSplitNumView();
            if (vw.ShowDialog() == System.Windows.Forms.DialogResult.OK) {
                var odr = gcOrder.GetFocusedDataSource<T_ERP_Order>();
                if (odr == null) return;
                if (odr.Num < 2) return;
                var newodr = odr.Copy();
                newodr.Num = vw.OrderNum;
                newodr.IsDynamicAdd = true;
                newodr.Guid = Guid.NewGuid();
                newodr.Reserved2 = odr.Guid.ToString();
                odr.Num = odr.Num - vw.OrderNum;
