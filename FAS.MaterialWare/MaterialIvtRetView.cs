...
{
    public partial class MaterialIvtRetView : MainSurface, ISurfacePermission
    {
        public MaterialIvtRetView()
        {
            InitializeComponent();
        }
        ...
        {
            pgr1.Caller = pgr2.Caller = pgr3.Caller = SerNoCaller_WL.Calr_MaterialIvtRet;
        ...
        {
            var et = gcUnAudit.GetFocusedDataSource<UltraDbEntity.T_ERP_MaterialIvtRet>();
            if (null == et) return;
            var kt = SerNoCaller_GC.Calr_MaterialIvtAdj.ExecSql("exec P_ERP_AuditMaterialIvtRet @0,@1", et.RetNo, this.CurUser);
            if (!kt.IsOK)
            {
        ...
        void barBtnInvalid_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            if (vw.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
                ...
                //作废操作
                var rd = SerNoCaller_GC.Calr_MaterialIvtAdj.ExecSql("exec P_ERP_InvalidMateriaRet @0,@1,@2",
                      et.RetNo, CurUser, et.InvalidRemark);
                if (!rd.IsOK)
                {
        ...
        void UnAudit()
        {
            ...
            pgr1.PrefixWhr = "select a.* from V_ERP_UnAuditMaterialIvtRet a";
            int idx = 0;
            ...
            {
                pgr1.Whrs.Add("a.RetNo = @" + (idx++).ToString());
        ...
        void Audit()
        {
            ...
            pgr2.PrefixWhr = "select a.* from V_ERP_AuditMaterialIvtRet a";
            int idx = 0;
            if (!string.IsNullOrEmpty(usredt.Text.Trim()))
            {
            ...
            if (!string.IsNullOrEmpty(txtadjno.Text.Trim()))
            {
                pgr2.Whrs.Add("a.RetNo = @" + (idx++).ToString());
        ...
        void Invalid()
        {
            ...
            pgr3.PrefixWhr = "select a.* from V_ERP_InvalidMaterialIvtRet a";
            int idx = 0;
            if (!string.IsNullOrEmpty(usredt.Text.Trim()))
            {
            ...
            if (!string.IsNullOrEmpty(txtadjno.Text.Trim()))
            {
                pgr3.Whrs.Add("a.RetNo = @" + (idx++).ToString());
