...
{
    public partial class WreckPriceIptView : DialogViewEx
    {
        public WreckPriceIptView()
        {
            InitializeComponent();
        ...
        {
            var dt = SqlHelper.ExecuteDataTable(this.ConnString, CommandType.StoredProcedure, "P_ERP_IptWreckPriceTpl");
            dt.Columns.Add("错误信息", typeof(System.String));
            gridControlEx1.DataSource = dt;
        }
        ...
        private void btnImp_Click(object sender, EventArgs e)
        {
            ...
            using (var db = new Database(this.ConnString))
            {
                try{
                    ...
                    var pros = db.Fetch<UltraDbEntity.T_ERP_WreckType>(" where isnull(IsUsing,0) = 1 ");
                    foreach (DataRow dr in dt.Rows)
                    {
                        pros.ForEach(j => {
                            var ipt = new T_ERP_IptPriceTemp
                            {
                                ...
                                ProcedureName = j.TypeName,
                                ProcedureGuid = j.Guid,
                                CostPrice = string.IsNullOrEmpty(dr[j.TypeName] == null
                                            ? "" : dr[j.TypeName].ToString())
                                            ? 0 : Convert.ToDecimal(dr[j.TypeName])
                            };
                            ipts.Add(ipt);
                        });
                    }
            ...
            SqlHelper.ExecuteNonQuery(this.ConnString, CommandType.StoredProcedure, "P_ERP_WreckPriceImport", prms);  
