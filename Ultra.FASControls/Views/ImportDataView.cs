...
namespace Ultra.FASControls.Views
{
    public partial class ImportDataView : DialogViewEx
    {
        ...
        public Ultra.FASControls.GridControlEx GridControl { get; set; }
        ...
        private string _FileFilter = "*.xls|*.xls";
        ...
        /// </summary>
        public Dictionary<string, string> DicKF
        {
            get
            {
                ...
                foreach (DevExpress.XtraGrid.Columns.GridColumn gc in gv.Columns)
                {
                    ...
                    if (b && strs.Where(j => j.EqualIgnorCase(gc.Caption)).Count() > 0)
                        continue;
                    if (!dic.ContainsKey(gc.Caption))
                        dic.Add(gc.Caption, gc.FieldName);
                }
                return dic;
            }
        }
            ...
            if (strs != null || strs.Length > 0)
            {
                ...
                if (null != gv)
                    foreach (var str in strs)
                        foreach (DevExpress.XtraGrid.Columns.GridColumn gc in gv.Columns)
                            if (gc.Caption.EqualIgnorCase(str) && gc.Visible)
                            {
                                gc.Visible = false;
                                gcEx.Add(gc);
                            }
        ...
        {
            if (null == GridControl) return;
            if (Ultra.Surface.Common.MsgBox.ShowYesNoMessage("确定要移除所选行吗?") == System.Windows.Forms.DialogResult.Yes)
            {
